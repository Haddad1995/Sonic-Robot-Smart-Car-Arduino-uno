...
#define F 0X3D9AE3F7  // FORWARD
#define B 0X1BC0157B  // BACK
#define L 0X8C22657B  // LEFT
#define R 0X449E79F // RIGHT
#define S 0X488F3CBB  // STOP
#define OA 0XE318261B // Obstacle Avoidance CH-
#define LT 0XEE886D7F // Line traking  CH+
...
#define carSpeed 90 //PWM(Motor speed/Speed)

LiquidCrystal_I2C lcd = LiquidCrystal_I2C(0x27, 16, 2);
unsigned int carSpeed_rocker = 90;
#define PIN_Servo 3
...
void line_teacking_mode(void)
{
  if (func_mode == LineTeacking)
  {
    ...
    { //Detecting in the middle infrared tube
      ...
      forward(false, 75); //Control motor：the car moving forward
      LT_PreMillis = millis();
    }
    else if (LineTeacking_Read_Right)
    { //Detecting in the right infrared tube

      right(false, 90); //Control motor：the car moving right
    ...
    {                   //Detecting in the left infrared tube
      left(false, 90); //Control motor：the car moving left
...
void obstacles_avoidance_mode(void)
{
  ...
  if (func_mode == ObstaclesAvoidance)
  {
    ...
    if (function_xxx(get_Distance, 0, 25))
    {
      ...
      */
      for (int i = 1; i < 6; i += 2) //1、3、5 Omnidirectional detection of obstacle avoidance status
      {
        ...
        else if (function_xxx(get_Distance, 0, 25)) //How many cm in the front have obstacles?
        {
          switc_ctrl += i;
        }
      }
    ...
    {
      forward(false, 90); //Control car forwar
    }
    while (switc_ctrl)
    {
      switch (switc_ctrl)
      ...
      case 6:
        ...
        break;
      case 3:
        left(false, 90); //Control car left
        switc_ctrl = 0;
        break;
      case 4:
        left(false, 90); //Control car left
      ...
      case 8:
      case 11:
        right(false, 90); //Control car right
        switc_ctrl = 0;
        break;
      case 9:
      case 10:
        back(false, 90); //Control car Car backwards
        switc_ctrl = 11;
        break;
      }
      ServoControl(90);
